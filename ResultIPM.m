...
clc;
close all;
total_tic = tic;             % <--- 新增：启动计时器
start_wall_time = datetime('now'); %<--- 新增：记录开始的墙上时间
seed = 3;
rng(seed);
...
% 初始参数:A0, B0, C0, D0, h1
%x0 = [10;2;2;8; 1.524];
x0 = [14.35;2.95;4.49;3.12; 1.524];%PSO转换值X0%这里的B0 C0与论文标注的c和b
%这里h1为固定值，不参与优化，仅作为接口传递
% 约束条件矩阵
A_paper = [0, 1, 1, 0, 0];  % B0 + C0 ≤ 8
b_paper = 8;

% 变量边界
lb = [1.0; 0.3; 0.1; 1.0; 1.524];
ub = [17.0; 8; 8; 17.0; 1.524];
...
try
    ...
    [x_opt, fval] = fmincon(@(x) ValueFun_WidebanPhase(x, target_freqs), ...
        x0, A_paper, b_paper, [], [], lb, ub, [], options);
...
fprintf('平均每次迭代耗时: %0.2f 秒 (基于记录的迭代数)\n', ...
    total_duration_seconds / length(function_evaluation_history));
fprintf('%s\n', repmat('=', 1, 50));
...
function modeValue = ValueFun_WidebanPhase(x, target_freqs)
...
function stop = optimization_logger(x, optimValues, state)
    persistent iteration_history;
    persistent previous_funccount;
    global function_evaluation_history;
    stop = false;
    ...
    switch state
        ...
        case 'iter'
            current_funccount = optimValues.funccount;
            current_iteration_evals = current_funccount - previous_funccount;
            previous_funccount = current_funccount;
            
            current_iter = struct();
            current_iter.iteration = optimValues.iteration;
            current_iter.total_funccount = optimValues.funccount;
            current_iter.current_iter_evals = current_iteration_evals;
            current_iter.fval = optimValues.fval;
            current_iter.stepsize = optimValues.stepsize;
            current_iter.firstorderopt = optimValues.firstorderopt;
            current_iter.iteration_parameters = x;
            current_iter.timestamp = datetime('now');
            
            % 获取最新评估的额外指标
            if ~isempty(function_evaluation_history)
                current_iter.total_evaluations_logged = length(function_evaluation_history);
                constraint_violations = sum([function_evaluation_history.constraint_violated]);
                current_iter.constraint_violations = constraint_violations;
                simulation_failures = sum(~[function_evaluation_history.success] & ~[function_evaluation_history.constraint_violated]);
                current_iter.simulation_failures = simulation_failures;
                ...
                current_iter.total_evaluations_logged = 0;
                current_iter.constraint_violations = 0;
                current_iter.simulation_failures = 0;
            ...
            save('optimization_history_wideband_phase.mat', 'iteration_history');
            
            fprintf('\n╔═══════════════════════════════════════════════════════════════╗\n');
            fprintf('║ 迭代 %d - 性能摘要                                           ║\n', optimValues.iteration);
            fprintf('╠═══════════════════════════════════════════════════════════════╣\n');
            fprintf('║ 函数评估: 本次%d, 累计%d                                    ║\n', ...
                    current_iteration_evals, optimValues.funccount);
            fprintf('║ Metric值: %.6f (越小越好)                              ║\n', ...
                    optimValues.fval);
            fprintf('║ 迭代参数: A0=%.3f, B0=%.3f, C0=%.3f, D0=%.3f, h1=%.3f  ║\n', x);
            fprintf('╠═══════════════════════════════════════════════════════════════╣\n');
            fprintf('║ 性能指标:                                                    ║\n');
            fprintf('║   • 有效带宽:     %.1f MHz (%.1f%%)                       ║\n', ...
        ...
        case 'done'
            ...
            if ~isempty(function_evaluation_history)
                ...
                fprintf('成功评估: %d (%.1f%%)\n', successful_evaluations, 100*successful_evaluations/length(function_evaluation_history));
            end
            
            if ~isempty(iteration_history)
                best_iter = iteration_history(end);
                fprintf('\n最优结果:\n');
                fprintf('最优参数: A0=%.4f, B0=%.4f, C0=%.4f, D0=%.4f, h1=%.4f\n', best_iter.iteration_parameters);
                ...
                save('final_optimization_result_wideband_phase.mat', 'best_iter', 'iteration_history');
            end
